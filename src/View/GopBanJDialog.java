...
BanDao DAOban = new BanDao();
    ...
    public void cnGopBan() {
        BanEntity ban = DAOban.selectById(idtable);
        List<BanChiTietEntity> listbct = DAObct.selectByIdBan(ban.getIdBan());
        BanEntity banchon = (BanEntity) cboBan.getSelectedItem();
        if (!listbct.isEmpty()) {
            for (BanChiTietEntity bct : listbct) {
    ...
    public void filltocbo() {
        List<BanEntity> list = DAOban.selectbanThuc();
        DefaultComboBoxModel model = (DefaultComboBoxModel) cboBan.getModel();
        model.removeAllElements();
        for (BanEntity ban : list) {
            List<BanChiTietEntity> listbct = DAObct.selectByIdBan(ban.getIdBan());
            if (!listbct.isEmpty()) {
                HoaDonEntity hd = DAOhd.selectById(listbct.get(0).getID_HoaDon());
