...
public class ThongtinBanJFrame extends javax.swing.JFrame {
    ...
    public ThongtinBanJFrame(List<BanChiTietEntity> bct, int ban) {
        initComponents();
        banct = bct;
        idban = ban;
        if (!banct.isEmpty()) {
            ...
            BanEntity bans = bandao.selectById(idban);
            lblSocho.setText(bans.getSoluongcho() + "");
        }
        getContentPane().setBackground(Color.CYAN);
    }
    static List<BanChiTietEntity> banct;
    static int idban;
    ...
    public void setinfor() {
        BanEntity ban = bandao.selectById(idban);
        lblSocho.setText(ban.getSoluongcho() + "");
        if (!banct.isEmpty()) {
            BanChiTietEntity banchitiet = banctdao.selectById(banct.get(0).getID_Ban(), banct.get(0).getID_HoaDon());
            lblthoigiantaodon.setText(Xdate.toString(banchitiet.getThoidiemCoNguoi(), "hh:mm:ss"));
            BanChiTietEntity banctdc = banctdao.selectBydonchinh(banchitiet.getID_HoaDon());
            lblban.setText(banctdc.getID_Ban() + "");
            int tienTong = 0;
            List<HoaDonEntity> listhd = hddao.selectCTT(idban);
            for (HoaDonEntity hoadon : listhd) {
                tienTong += hoadon.getThanhTien();
            }
            lblTongTien.setText(tienTong + "");
            HoaDonEntity hd = hddao.selectById(banctdc.getID_HoaDon());
