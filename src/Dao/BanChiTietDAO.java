...
public class BanChiTietDAO implements InterfaceBanchitiet{
    ...
    @Override
    public void insert(BanChiTietEntity Entity) {
    ...
    @Override
    public BanChiTietEntity selectById(int idban,int idHoadon) {
        List<BanChiTietEntity> list = this.selectBySql(SELECT_BY_ID_SQL, idHoadon,idban);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    @Override
    public List<BanChiTietEntity> selectAll() {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public List<BanChiTietEntity> selectBySql(String sql, Object... args) {
        List<BanChiTietEntity> list = new ArrayList<>();
        try {
            ResultSet rs = JDBCHeper.query(sql, args);
            while (rs.next()) {
                BanChiTietEntity Entity = new BanChiTietEntity();
    ...
    @Override
    public List<BanChiTietEntity> selectByIdBan(int idban) {
        return selectBySql(SELECT_BY_ID_Ban, idban);
    }

    @Override
    public List<BanChiTietEntity> selectByIdHD(int idHoadon) {
    ...
    public BanChiTietEntity selectBydonchinh(int id) {
        List<BanChiTietEntity> list = this.selectBySql(SELECT_donchinh, id);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
