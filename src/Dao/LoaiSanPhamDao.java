...
public class LoaiSanPhamDao implements InterfaceLoaiSanPham{
    ...
    @Override
    public void insert(LoaiSanPhamEntity lsp) {
        JDBCHeper.update(insert, lsp.getID_LoaiSP(),lsp.getTenLoai());
    }

    @Override
    public void update(LoaiSanPhamEntity lsp) {
    ...
    @Override
    public List<LoaiSanPhamEntity> selectAll() {
        return selectBySQL(selectAll);
    }

    @Override
    public LoaiSanPhamEntity selectID(String id) {
        return selectBySQL(selectByid,id).get(0);
    }

    @Override
    public List<LoaiSanPhamEntity> selectBySQL(String sql, Object... agrs) {
        List<LoaiSanPhamEntity> list = new ArrayList<>();
        try {
            ResultSet rs = JDBCHeper.query(sql, agrs);
            while (rs.next()) {
                LoaiSanPhamEntity lsp = new LoaiSanPhamEntity();
    ...
    public LoaiSanPhamEntity selectId_LSP(String id){
        List<LoaiSanPhamEntity> list = this.selectBySQL(selectByid, id);
        if(list.isEmpty()){
            return null;
        }
        return list.get(0);
