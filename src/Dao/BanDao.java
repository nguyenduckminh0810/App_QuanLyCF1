...
//        "AND dbo.HoaDon.TTThanhtoan = 0 AND Donchinh = 1 AND dbo.HoaDon.ID_Hoadon = ?";
    @Override
    public void insert(BanEntity Entity) {
    ...
    @Override
    public BanEntity selectById(int id) {
        List<BanEntity> list = this.selectBySql(SELECT_BY_ID_SQL, id);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
...
//    }
    ...
    @Override
    public List<BanEntity> selectAll() {
        return this.selectBySql(SELECT_ALL_SQL);
    }
    public List<BanEntity> selectAllAO() {
        return this.selectBySql(SELECT_ALL_AO_SQL);
    }
    @Override
    public List<BanEntity> selectBySql(String sql, Object... args) {
        List<BanEntity> list = new ArrayList<>();
        try {
            ResultSet rs = JDBCHeper.query(sql, args);
            while (rs.next()) {
                BanEntity Entity = new BanEntity();
    ...
    public BanEntity selectIDHD(int maHD){
        //return selectBySql(SELECT_BY_ID_HD, maHD).get(0);
        List<BanEntity> list = this.selectBySql(SELECT_BY_ID_HD, maHD);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public List<BanEntity> selectbanTrong(){
        return selectBySql(SELECT_ALL_Ban_Trong_ALL);
    }
    
    public List<BanEntity> selectbanThuc(){
