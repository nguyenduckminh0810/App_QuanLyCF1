...
public class SanPhamDao implements InterfaceSanPham{
   ...
   String insert = "insert into dbo.SanPham(ID_Sanpham,TenSP,Gia,ID_DonviSP,ID_LoaiSP,Trangthai) values(?,?,?,?,?,?)";
//String insert = "insert into dbo.SanPham(ID_Sanpham,TenSP,Gia,ID_DonviSP,ID_LoaiSP,Trangthai) values(?,?,?,DV1,?,?)";
...
String update_size2 = "update SanPham set ID_DonviSP=?";
    String update = "update SanPham set TenSP = ? , Gia =? ,ID_DonviSP=?,"
            + "ID_LoaiSP =?, Trangthai=? where ID_Sanpham=?";
    ...
    @Override
    public SanPhamEntity selectID(String id) {
        List<SanPhamEntity> list = selectBySQL(selectID, id);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    @Override
    public List<SanPhamEntity> selectBySQL(String sql, Object... agrs) {
        List<SanPhamEntity> list_sanpham = new ArrayList<>();
        try {
    ...
    public List<SanPhamEntity> select_Trangthai() {
        return selectBySQL(select_Trangthai2);
    }

    public List<SanPhamEntity> selectByKeyword(String keyword, String LoaiSP) {
        String sql = "SELECT * FROM dbo.SanPham WHERE TenSP LIKE ? AND ID_LoaiSP = ? AND Trangthai = 1 ";
        return selectBySQL(sql, "%" + keyword + "%", LoaiSP);
    }

    public SanPhamEntity selecteByIDSP(String key) {
        List<SanPhamEntity> list = (List<SanPhamEntity>) selectBySQL(SElECT_SQL_IDSP, key);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
    ...
    public List<SanPhamEntity> selectALLL() {
        return selectBySQL(seleteAll);
    }

    public List<SanPhamEntity> Select_ByName(String name) {  // tìm theo tên sản phẩm
