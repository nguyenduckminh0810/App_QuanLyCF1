...
public class QuenMatKhauJDialog extends javax.swing.JDialog {
    ...
    private void btnguimaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnguimaActionPerformed
        ...
        try {
            btnguima.setEnabled(true);//Vô hiệu hoá nút lấy mã tránh trường hợp người dùng spam
            txtMaxacnhan.setEnabled(true);

            //Cấu hình thuộc tính gửi mail qua Gmail
            ...
            //Email dùng để gửi mail
            String userName = "testquanlycafeda@gmail.com";
            String password = "tokvytsaxqmxiuus";
            //tạo 1 phiên làm việc với các thuộc tính cấu hình được cung cấp trong 'p' và thông tin tk mk được xác thực
            Session source = Session.getInstance(p, new javax.mail.Authenticator() {
                protected javax.mail.PasswordAuthentication getPasswordAuthentication() {
                    return new javax.mail.PasswordAuthentication(userName, password);
                }
            });

            //Gửi OTP về mail người dùng
            String from = userName;//Gửi từ mail
            String to = txtEmail.getText();//Gửi đến mail
            code = this.random();//Tạo OTP random
            String title = "Quên mật khẩu - App Quản Lý Cafe";//Tiêu đề của mail

            //Nội dung của mail
            Message msg = new MimeMessage(source);//Tạo đối tượng cấu hình email
            msg.setFrom(new InternetAddress(from));//Set địa chỉ gửi mail
            msg.setRecipients(Message.RecipientType.TO, InternetAddress.parse(to));//Set địa chỉ nhận mail
            ...
            txtMaxacnhan.setEnabled(true);//Cho phép người dùng nhập OTP trong textfield
        } catch (MessagingException e) {//Thông báo lỗi 
            JOptionPane.showMessageDialog(this, "Gửi mã OTP thất bại! Vui lòng kiểm tra kết nối internet và thử lại.", "Lỗi gửi email", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        } finally {
            btnguima.setEnabled(true);//Kích hoạt lại nút Lấy mã trong mọi trường hợp
        }
    ...
    //Check mail có đúng định dạng hay không
